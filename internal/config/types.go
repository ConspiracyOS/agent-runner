...
type AgentConfig struct {
	...
	// Deprecated: use Runner. Kept for backwards compatibility.
	CLI     string   `toml:"cli"`
...
func (c *Config) ResolvedAgent(name string) AgentConfig {
	for _, a := range c.Agents {
		if a.Name == name {
			...
			// Resolve CLI/Runner: "cli" is the deprecated name for "runner"
			if resolved.Runner == "" && resolved.CLI != "" {
				resolved.Runner = resolved.CLI
			}
			...
			if resolved.APIKeyEnv == "" {
				resolved.APIKeyEnv = "CON_API_KEY"
			}
