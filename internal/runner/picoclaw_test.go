...
func TestBuildPicoConfig(t *testing.T) {
	...
	// Provider: OpenRouter should be configured with the env key
	...
	// RestrictToWorkspace should be false (Linux perms are the sandbox)
	if pcfg.Agents.Defaults.RestrictToWorkspace {
		t.Error("expected RestrictToWorkspace=false")
	}

	// MaxToolIterations should be 50
...
func TestBuildPicoConfigProviderPriority(t *testing.T) {
	...
	// No env vars set — no provider should be configured
	...
	// Anthropic takes priority over OpenAI when OpenRouter is not set
